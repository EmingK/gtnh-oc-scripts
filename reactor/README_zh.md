...
- 全局控制：通过外部红石控制反应堆运行，并连接至能量存储实现按需发电。
...
储电GT机器如兰波顿、能量站等机器对应一个EU存储实例。你可以配置实例的如下属性：

- 名称：EU存储机器名称,可以在运行界面显示。
- 设备：配置存储机器的地址。
- 上阈值：在机器EU量占总容量的比例超过上阈值时应该停机。
- 下阈值：在机器EU量占总容量的比例低于下阈值时应该开机。

### EU储量控制启停逻辑

EU储量控制界面包含两个可配置选项：开机逻辑和停机逻辑。配置为 `与` 会在所有EU存机器满足对应的阈值时触发，配置为 `或` 会在任意一EU存储机器满足对应的阈值时触发。
